...
interface SquareHourlyChartProps {
  hourlyActivity: number[];
  peakHour: number;
  colors: ThemeColors;
  scale?: number;
...
export function SquareHourlyChart({ hourlyActivity, peakHour, colors, scale = 1 }: SquareHourlyChartProps) {
  const s = createScaler(scale);
  const threeHourBlocks: number[] = [];
  for (let i = 0; i < 24; i += 3) {
    threeHourBlocks.push(hourlyActivity[i] + hourlyActivity[i + 1] + hourlyActivity[i + 2]);
  }
  const maxBlock = Math.max(...threeHourBlocks);
  const peakBlockIndex = Math.floor(peakHour / 3);
  ...
  return (
    <div style={flexColumn(s(spacing[1]))}>
      <div style={{ ...flexRow(barGap), alignItems: "flex-end", height: barHeight }}>
        {threeHourBlocks.map((val, i) => {
          const heightPercent = maxBlock > 0 ? val / maxBlock : 0;
          const h = Math.max(2, Math.round(heightPercent * barHeight));
          const isPeak = i === peakBlockIndex;
          return <div key={i} style={{ width: barWidth, height: h, backgroundColor: isPeak ? colors.accent.primary : colors.heatmap.level2, borderRadius: s(2) }} />;
        })}
      </div>
      <div style={{ ...flexRow(), justifyContent: "space-between", width: (barWidth * 8) + (barGap * 7) }}>
        <span style={{ fontSize: s(typography.size.xs), color: colors.text.muted }}>0</span>
        <span style={{ fontSize: s(typography.size.xs), color: colors.text.muted }}>12</span>
        <span style={{ fontSize: s(typography.size.xs), color: colors.text.muted }}>24</span>
      </div>
...
interface StoryCodingHoursChartProps {
  hourlyStats: { peakHour: number };
  hourlyActivity: number[];
  colors: ThemeColors;
  scale: number;
  expanded?: boolean;
...
export function StoryCodingHoursChart({ hourlyStats, hourlyActivity, colors, scale, expanded = false }: StoryCodingHoursChartProps) {
  const s = createScaler(scale);
  const twoHourBlocks: number[] = [];
  for (let i = 0; i < 24; i += 2) {
    twoHourBlocks.push(hourlyActivity[i] + hourlyActivity[i + 1]);
  ...
  const maxBlock = Math.max(...twoHourBlocks);
  const peakBlockIndex = Math.floor(hourlyStats.peakHour / 2);
  const barHeight = expanded ? s(56) : s(40);
  const barWidth = expanded ? s(24) : s(18);
  const barGap = expanded ? s(8) : s(6);
  ...
  return (
    <div style={flexColumn(s(spacing[2]))}>
      <div style={{ ...flexRow(barGap), alignItems: "flex-end", height: barHeight }}>
        {twoHourBlocks.map((val, i) => {
          const heightPercent = maxBlock > 0 ? val / maxBlock : 0;
          const h = Math.max(s(3), Math.round(heightPercent * barHeight));
          const isPeak = i === peakBlockIndex;
          return <div key={i} style={{ width: barWidth, height: h, backgroundColor: isPeak ? colors.accent.primary : colors.heatmap.level2, borderRadius: s(3) }} />;
        })}
      </div>
      <div style={{ ...flexRow(), justifyContent: "space-between", width: (barWidth * 12) + (barGap * 11) }}>
        <span style={{ fontSize: s(typography.size.xs), color: colors.text.muted }}>0</span>
        <span style={{ fontSize: s(typography.size.xs), color: colors.text.muted }}>12</span>
        <span style={{ fontSize: s(typography.size.xs), color: colors.text.muted }}>24</span>
      </div>
