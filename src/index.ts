...
OPTIONS:
  ...
  --format <default|square|story>
                     Image layout format (default: default)
                       default: 1500x1700 - standard layout
                       square:  1024x1024 - compact 1:1 for avatars
                       story:   1080x1920 - 9:16 vertical for IG/TikTok stories
...
EXAMPLES:
  ...
  ampcode-wrapped --format square    # Generate 1:1 square image
  ampcode-wrapped --format story     # Generate 9:16 story image
`);
}
...
async function main() {
  ...
  if (values.format === "square" || values.format === "story") {
    layoutFormat = values.format;
  } else if (values.format && values.format !== "default") {
    const errorMsg = `Invalid format "${values.format}". Valid options: default, square, story`;
    if (quietMode) {
